...
# Definição dos dados de hoje para evitar NameError
hoje_str = datetime.now(fuso_br).strftime('%d/%m/%Y')
clis_hoje = [a for a in agnd if a.get('data') == hoje_str]
...
with col_ops_l:
    ...
    with t1:
        with st.form("form_ag_v4", clear_on_submit=True):
            cli_n = st.selectbox("Selecionar Cliente", [c['nome'] for c in clis], key="sel_c_v4") if clis else None
            srv_n = st.selectbox("Serviço", [s['nome'] for s in srvs], key="sel_s_v4") if srvs else None
            c_d, c_h = st.columns(2)
            d_val = c_d.date_input("Data", key="date_v4")
            h_val = c_h.time_input("Horário", key="time_v4")
    ...
    with t2:
        with st.form("form_cli_v4", clear_on_submit=True):
            nome_c = st.text_input("Nome do Cliente", key="n_c_v4")
            tel_c = st.text_input("WhatsApp", key="t_c_v4")
    ...
    with t3:
        with st.form("form_srv_v4", clear_on_submit=True):
            nome_s = st.text_input("Nome do Serviço", key="n_s_v4")
            preco_s = st.number_input("Preço", min_value=0.0, step=10.0, key="p_s_v4")
    ...
    with t4:
        with st.form("form_cx_v4", clear_on_submit=True):
            desc_cx = st.text_input("Descrição", key="d_cx_v4")
            valor_cx = st.number_input("Valor", min_value=0.0, key="v_cx_v4")
            tipo_cx = st.selectbox("Tipo", ["Entrada", "Saída"], key="t_cx_v4")
